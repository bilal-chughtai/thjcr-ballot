...
class RavenResponse(object):
    ...
    def __init__(self, response):
        ...
        for search, replace_with in replace.items():
            self.sig = self.sig.replace(search, replace_with)

        try:
            self.sig = b64decode(self.sig, validate=False)
    ...
    def check_timestamp(self):
        now = datetime.utcnow()
        diff = min((self.issue - now).seconds, (now-self.issue).seconds)

        if diff > 60: #get_config('RAVEN_RESPONSE_TIMESTAMP_DIFF'):
...
class RavenRequest(object):
    ...
    def redirect_url(self):
        ...
        return ('https://raven.cam.ac.uk/auth/authenticate.html' + '?' + urlencode(params))
